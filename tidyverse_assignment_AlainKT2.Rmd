...
### Add ons
```{r}
ggplot(wine_final, aes(x=points, y=price))+ geom_point()
```
...
```{r}
 med.price <- wine_final %>% filter(variety %in% 'Chardonnay', province %in% 'California', points==91) %>% group_by(variety,points) %>% summarise( median(price))
```
